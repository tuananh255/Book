...
const createOrder = asyncHandle(async (req, res) => {
  const { shippingInfo, orderItems, totalPrice, paymentMethod } = req.body;
  const { _id } = req.user;
  console.log(shippingInfo, orderItems, totalPrice, _id, paymentMethod);
  ...
  try {
    ...
    // Update the package product quantities
    for (const item of orderItems) {
      const { productId, quantity } = item;
      
      const packageProduct = await packageProductModel.findOne({ productId });

      if (packageProduct) {
        packageProduct.quantity -= quantity;
...
module.exports = {addUser,loginUser,getAllUsers,getsignUser,deletesignUser
    ,updateUser,handleRefreshToken,logout,updatePassword
    ,loginAdmin,userCart,sendEmail,
